...
describe("Token contract", function () {
    ...
    let revShare;
    let factory;
    let revShareContract: Contract;
    let revShareFactory: Contract;
    ...
    beforeEach(async function () {
        ...
        RevShare = await ethers.getContractFactory("RevShareCertificate");
        [owner, addr1, addr2, ...addrs] = await ethers.getSigners();

        // To deploy our contract, we just have to call Token.deploy() and await
        // for it to be deployed(), which happens onces its transaction has been
        // mined.
        revShare = await revShareContract.deploy();
    });
    ...
    describe("Deployment", function () {
        ...
        it("should deploy the factory and be able to deploy a new ", async function () {
            revShare = await revshareFactory.deployRevShareAgreement("Test", 3000, owner, addr1);
            expect(revShare).to.equal(owner.address);
        });
